...
module main_control_new(input clk,
  input rst,
  input CS1,
  input SDI1,
  input clk2,
...
always @ (negedge clk) begin
	VSAF_EN <= fp_vsa_test_mode | fp_mode;
	VSAB_EN <= bp_vsa_test_mode | bbp_mode | bmp_mode;
end

genvar i;
for (i=0; i<64; i=i+1) begin  : WTA_SEL0_logic
    assign WTA_SEL0[i] = (wta_va_decoded[i] & wta_test_mode) | (bp_wta_sel_test[i] & bp_wta_test_mode) | ((bbp_mode | bwp_mode) & memvar_mask[i]);                          
    assign WTA_SEL1[i] = (wta_vb_decoded[i] & wta_test_mode) | (bp_wta_sel_test[i] & bp_wta_test_mode) | ((bbp_mode | bwp_mode) & memvar_mask[i]);
