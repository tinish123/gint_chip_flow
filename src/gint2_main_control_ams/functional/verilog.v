...
//wire unsat;
wire [63:0] variable_mask, randVar_sel_l0, candVar_list;
...
reg single_flip, restart_flag1, restart_flag2, restart_flag3;
reg [18:0] flip_counter1, flip_counter2, flip_counter3;
reg [7:0] restart_counter;

genvar pp;
for (pp=0; pp<64; pp=pp+1) begin  : Cand_VAR_logic
    assign candVar_list[pp] = PBIT_OUT_reg[pp] & ~VSAB_OUT_reg[pp] & variable_mask[pp];
    assign VAR_flipped[pp] = ((~single_flip) & (((flip_mode1 & VAR1[pp]) | (flip_mode2 & VAR2[pp]) | (flip_mode3 & VAR3[pp])) ^ candVar_list[pp])) | ((single_flip) & (((flip_mode1 & VAR1[pp]) | (flip_mode2 & VAR2[pp]) | (flip_mode3 & VAR3[pp])) ^ randVar_sel_l0[pp]));
end

randVarSelect randVarSelect_UUT (candVar_list,variable_mask,prngbits_randVar,randVar_sel_l0,candVar_present);
...
assign PBIT_EN = clk_p & (bp_mode1 | bp_mode2 | bp_mode3 | pbit_test_int_mode | pbit_test_ext_mode);
//assign FP_PUn = ~(fp_mode1 | fp_mode2 | fp_mode3 | fp_vsa_test_mode);

genvar i;
for (i=0; i<64; i=i+1) begin  : PBIT_SEL0_logic
    assign MB_SEL[i] = ((pbit_test_int_mode | bp_vsa_test_mode) & mb_sel_test[i]) | (bp_mode1 & VAR1[i]) | (bp_mode2 & VAR2[i]) | (bp_mode3 & VAR3[i]);
    assign PBIT_SEL[i] = (pbit_test_int_mode & 1'b1) | (pbit_test_ext_mode & pbit_sel_test[i]) | ((bp_mode1 | bp_mode2 | bp_mode3) & variable_mask[i]);
    assign BP_PUnS[2*i] = ~ (((pbit_test_int_mode | pbit_test_ext_mode | bp_vsa_test_mode) | variable_mask[i] & (bp_mode1 | bp_mode2 | bp_mode3)) & ~MB_SEL[i]);
    assign BP_PUnS[2*i+1] = ~ (((pbit_test_int_mode | pbit_test_ext_mode | bp_vsa_test_mode) | variable_mask[i] & (bp_mode1 | bp_mode2 | bp_mode3)) & MB_SEL[i]);
    assign BP_PUnZ[2*i] = ~ (((pbit_test_int_mode | pbit_test_ext_mode) | variable_mask[i] & (bp_mode1 | bp_mode2 | bp_mode3)) & MB_SEL[i]);
    assign BP_PUnZ[2*i+1] = ~ (((pbit_test_int_mode | pbit_test_ext_mode) | variable_mask[i] & (bp_mode1 | bp_mode2 | bp_mode3)) & ~MB_SEL[i]);
    ...
    assign rp3[i] = ((bp_mode1 | bp_mode2 | bp_mode3) & rng_buffer[8*i]) | ((pbit_test_int_mode | pbit_test_ext_mode) & pbit_rp_test[3]);
    assign rp2[i] = ((bp_mode1 | bp_mode2 | bp_mode3) & rng_buffer[8*i+1]) | ((pbit_test_int_mode | pbit_test_ext_mode) & pbit_rp_test[2]);
    assign rp1[i] = ((bp_mode1 | bp_mode2 | bp_mode3) & rng_buffer[8*i+2]) | ((pbit_test_int_mode | pbit_test_ext_mode) & pbit_rp_test[1]);
    assign rp0[i] = ((bp_mode1 | bp_mode2 | bp_mode3) & rng_buffer[8*i+3]) | ((pbit_test_int_mode | pbit_test_ext_mode) & pbit_rp_test[0]);
    
    assign rn3[i] = ((bp_mode1 | bp_mode2 | bp_mode3) & rng_buffer[8*i+4]) | ((pbit_test_int_mode | pbit_test_ext_mode) & pbit_rn_test[3]);
    assign rn2[i] = ((bp_mode1 | bp_mode2 | bp_mode3) & rng_buffer[8*i+5]) | ((pbit_test_int_mode | pbit_test_ext_mode) & pbit_rn_test[2]);
    assign rn1[i] = ((bp_mode1 | bp_mode2 | bp_mode3) & rng_buffer[8*i+6]) | ((pbit_test_int_mode | pbit_test_ext_mode) & pbit_rn_test[1]);
    assign rn0[i] = ((bp_mode1 | bp_mode2 | bp_mode3) & rng_buffer[8*i+7]) | ((pbit_test_int_mode | pbit_test_ext_mode) & pbit_rn_test[0]);
end
...
for (m=0; m<256; m=m+1) begin  : BP_WLm_logic
    assign RWLBS_pre[m] = ((pbit_test_int_mode | bp_vsa_test_mode) & bp_wls_test[m]) | ((bp_mode1 | bp_mode2 | bp_mode3) & S[m]);
    assign RWLBZ_pre[m] = (pbit_test_int_mode & bp_wlz_test[m]) | ((bp_mode1 | bp_mode2 | bp_mode3) & Z[m]);
...
parameter size = 5;
parameter s_IDLE = 5'b00000, s_DIN_DECODE = 5'b00001, s_PBIT_TEST_EXT = 5'b00010, s_FP_VSA_TEST = 5'b00011, s_BP_VSA_TEST = 5'b00100, s_PBIT_TEST_INT = 5'b00101, s_PRNG_TEST1 = 5'b00110;
parameter s_PROG_BL1 = 5'b00111, s_PROG_WL = 5'b01000, s_PROG_BL2 = 5'b01001, s_SOLVE_FP = 5'b01010, s_SOLVE_BP = 5'b01011, s_SOLVE_FLIP = 5'b01100, s_SOLVE_RNG_INIT = 5'b01101, s_SOLVE_RESTART = 5'b01110, s_PRNG_TEST2 = 5'b01111;
